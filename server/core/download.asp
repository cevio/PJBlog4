...
define(function(require, exports, module){
	var download = function(url, path, options){
		this.fso = require.async("FSO");
		this.xmlhttp = new ActiveXObject(config.nameSpace.xmlhttp);
		this.url = url;
		this.path = path;
		this.start = 0;
		this.length = 0;
	...
	download.prototype.checkFile = function(){
		...
		var fsoObject = this.fso.createFSO();
		if ( this.fso.exsit(this.pathname) ){ this.start = fsoObject.getfile(selector.lock(this.pathname)).size; };
		fsoObject = null;
	...
	download.prototype.range = function(){
		...
		var stream = new ActiveXObject(config.nameSpace.stream);
			...
			if ( this.fso.exsit(this.pathname) ){
				stream.LoadFromFile(selector.lock(this.pathname));
			}
			stream.Position = this.fileStart;
			stream.Write(this.xmlhttp.responseBody);
			stream.SaveToFile(selector.lock(this.pathname), 2);
			stream.Close();
			stream = null;
	...
	return function(url, path, options){
